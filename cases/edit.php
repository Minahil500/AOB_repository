<?php

session_start();

include "../config/db.php";

if (!isset($_GET['id']) || !is_numeric($_GET['id'])) {

    header("Location: index.php");
    exit();

}

$case_id = (int) $_GET['id'];

$query = "
    SELECT *
    FROM cases
    WHERE id = $case_id
    LIMIT 1
";

$result = mysqli_query($conn, $query);

if (!$result) {

    die("Database Error: " . mysqli_error($conn));

}

$case = mysqli_fetch_assoc($result);

if (!$case) {

    die("Case not found.");

}

$firms_query = "
    SELECT id, firm_name
    FROM firms
    WHERE status = 'active'
    ORDER BY firm_name ASC
";

$firms_result = mysqli_query($conn, $firms_query);

$error = "";

if ($_SERVER["REQUEST_METHOD"] === "POST") {

    $case_number = trim($_POST['case_number'] ?? '');
    $firm_id = (int) ($_POST['firm_id'] ?? 0);
    $case_title = trim($_POST['case_title'] ?? '');
    $case_type = trim($_POST['case_type'] ?? '');
    $regulation_violated = trim($_POST['regulation_violated'] ?? '');
    $assigned_officer = trim($_POST['assigned_officer'] ?? '');
    $court_name = trim($_POST['court_name'] ?? '');
    $priority = trim($_POST['priority'] ?? '');
    $status = trim($_POST['status'] ?? '');
    $has_court_order = isset($_POST['has_court_order']) ? 1 : 0;
    $next_followup_date = !empty($_POST['next_followup_date'])
        ? $_POST['next_followup_date']
        : null;

    if (
        $case_number === "" ||
        $firm_id <= 0 ||
        $case_title === "" ||
        $case_type === ""
    ) {

        $error = "Please fill all required fields.";

    } else {

        $check_query = "
            SELECT id
            FROM cases
            WHERE case_number = ?
            AND id != ?
            LIMIT 1
        ";

        $check_stmt = mysqli_prepare(
            $conn,
            $check_query
        );

        mysqli_stmt_bind_param(
            $check_stmt,
            "si",
            $case_number,
            $case_id
        );

        mysqli_stmt_execute($check_stmt);

        $check_result = mysqli_stmt_get_result($check_stmt);

        if (mysqli_num_rows($check_result) > 0) {

            $error = "Another case already uses this Case Number.";

        } else {

            $update_query = "
                UPDATE cases
                SET
                    case_number = ?,
                    firm_id = ?,
                    case_title = ?,
                    case_type = ?,
                    regulation_violated = ?,
                    assigned_officer = ?,
                    court_name = ?,
                    priority = ?,
                    status = ?,
                    has_court_order = ?,
                    next_followup_date = ?
                WHERE id = ?
            ";

            $update_stmt = mysqli_prepare(
                $conn,
                $update_query
            );

            if (!$update_stmt) {

                die(
                    "Prepare failed: " .
                    mysqli_error($conn)
                );

            }

            mysqli_stmt_bind_param(
                $update_stmt,
                "sisssssssisi",
                $case_number,
                $firm_id,
                $case_title,
                $case_type,
                $regulation_violated,
                $assigned_officer,
                $court_name,
                $priority,
                $status,
                $has_court_order,
                $next_followup_date,
                $case_id
            );

            if (mysqli_stmt_execute($update_stmt)) {

                header("Location: index.php");
                exit();

            } else {

                $error = "Update failed: " . mysqli_stmt_error($update_stmt);

            }

        }

    }

}

include "../includes/header.php";
include "../includes/sidebar.php";

?>

<main>

<h1>
Edit Case
</h1>

<p>
Update the legal case information.
</p>

<?php if ($error !== "") { ?>

<div class="form-error">

    <?php
    echo htmlspecialchars($error);
    ?>

</div>

<?php } ?>

<section>

<form method="POST">

<div class="form-group">

<label>
Case Number *
</label>

<input
    type="text"
    name="case_number"
    value="<?php
        echo htmlspecialchars(
            $_POST['case_number']
            ?? $case['case_number']
        );
    ?>"
    required
>

</div>

<div class="form-group">

<label>
Firm *
</label>

<select
    name="firm_id"
    required
>

<option value="">
Select Firm
</option>

<?php

if ($firms_result) {

    while ($firm = mysqli_fetch_assoc($firms_result)) {

?>

<option
    value="<?php echo $firm['id']; ?>"
    <?php

    echo (
        ($_POST['firm_id'] ?? $case['firm_id'])
        == $firm['id']
    )
    ? 'selected'
    : '';

    ?>
>

<?php
echo htmlspecialchars($firm['firm_name']);
?>

</option>

<?php

    }

}

?>

</select>

</div>

<div class="form-group">

<label>
Case Title *
</label>

<input
    type="text"
    name="case_title"
    value="<?php
        echo htmlspecialchars(
            $_POST['case_title']
            ?? $case['case_title']
        );
    ?>"
    required
>

</div>

<div class="form-group">

<label>
Case Type *
</label>

<select
    name="case_type"
    required
>

<?php

$case_types = [
    "Enforcement",
    "Legal",
    "Appeal",
    "Disciplinary",
    "Other"
];

$current_case_type =
    $_POST['case_type']
    ?? $case['case_type'];

foreach ($case_types as $type) {

?>

<option
    value="<?php echo htmlspecialchars($type); ?>"
    <?php
    echo ($current_case_type === $type)
        ? 'selected'
        : '';
    ?>
>

<?php
echo htmlspecialchars($type);
?>

</option>

<?php

}

?>

</select>

</div>

<div class="form-group">

<label>
Regulation Violated
</label>

<textarea
    name="regulation_violated"
    rows="4"
><?php

echo htmlspecialchars(
    $_POST['regulation_violated']
    ?? $case['regulation_violated']
    ?? ''
);

?></textarea>

</div>

<div class="form-group">

<label>
Assigned Officer
</label>

<input
    type="text"
    name="assigned_officer"
    value="<?php

        echo htmlspecialchars(
            $_POST['assigned_officer']
            ?? $case['assigned_officer']
            ?? ''
        );

    ?>"
>

</div>

<div class="form-group">

<label>
Court Name
</label>

<input
    type="text"
    name="court_name"
    value="<?php

        echo htmlspecialchars(
            $_POST['court_name']
            ?? $case['court_name']
            ?? ''
        );

    ?>"
>

</div>

<div class="form-group">

<label>
Priority
</label>

<select
    name="priority"
>

<?php

$priorities = [
    "Low",
    "Medium",
    "High",
    "Critical"
];

$current_priority =
    $_POST['priority']
    ?? $case['priority'];

foreach ($priorities as $priority) {

?>

<option
    value="<?php echo htmlspecialchars($priority); ?>"
    <?php
    echo ($current_priority === $priority)
        ? 'selected'
        : '';
    ?>
>

<?php
echo htmlspecialchars($priority);
?>

</option>

<?php

}

?>

</select>

</div>

<div class="form-group">

<label>
Status
</label>

<select
    name="status"
>

<?php

$statuses = [
    "Draft",
    "Open",
    "Under Review",
    "Referred to Court",
    "Closed",
    "Archived"
];

$current_status =
    $_POST['status']
    ?? $case['status'];

foreach ($statuses as $case_status) {

?>

<option
    value="<?php echo htmlspecialchars($case_status); ?>"
    <?php
    echo ($current_status === $case_status)
        ? 'selected'
        : '';
    ?>
>

<?php
echo htmlspecialchars($case_status);
?>

</option>

<?php

}

?>

</select>

</div>

<div class="form-group">

<label>

<input
    type="checkbox"
    name="has_court_order"
    value="1"

    <?php

    $current_court_order =
        $_POST['has_court_order']
        ?? $case['has_court_order'];

    echo ($current_court_order == 1)
        ? 'checked'
        : '';

    ?>
>

Has Court Order

</label>

</div>

<div class="form-group">

<label>
Next Follow-up Date
</label>

<input
    type="date"
    name="next_followup_date"
    value="<?php

        echo htmlspecialchars(
            $_POST['next_followup_date']
            ?? $case['next_followup_date']
            ?? ''
        );

    ?>"
>

</div>

<div class="form-actions">

<a
    href="index.php"
    class="btn-secondary"
>
Cancel
</a>

<button
    type="submit"
    class="btn-primary"
>
Save Changes
</button>

</div>

</form>

</section>

</main>

<?php

include "../includes/footer.php";

?>